...
const TableReservationForm = ({ user, onSuccess, userBookings }) => {
    ...
    const handleApplyCoupon = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/public/coupons/validate?code=${encodeURIComponent(code)}&amount=${currentTotal}&appliesTo=all`);
    ...
    useEffect(() => {
        const fetchMenu = async () => {
            try {
                const res = await fetch('http://localhost:5000/api/public/menu');
                if (res.ok) {
                    const data = await res.json();
                    setMenuItems(data);
                }
